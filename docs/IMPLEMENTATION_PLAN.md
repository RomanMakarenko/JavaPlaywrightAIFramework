...
Current position: **Phase 12 implemented — Definition of Done complete**; **Phase 15 (Jira bug pipeline) implemented and verified end-to-end** 2026-08-23 (real Bug/Task ticket `SCRUM-10` filed with a failure screenshot; a second run **reused** it instead of duplicating; full suite 11/11 green). Phases 14 (env/browser) and 15 added 2026-08-23. Repo pushed to GitHub 2026-08-23; CI runs green (full suite on `main`, style gate, Allure report, `gh-pages` deploy) and the **Allure report is live** at `https://romanmakarenko.github.io/JavaPlaywrightAIFramework/`. Jira CI secrets set; **`Bug` issue type added to the SCRUM scheme via the UI** (2026-08-23, verified via API — id `10039`) so the pipeline files literal `Bug` tickets (see 15.6). **Remaining (owner side): rotate the Jira API token** exposed earlier in the pre-commit `TASK_SPECK.md`/`EVIDENCE.md`.
...
- [x] CI green on a test PR; Allure report accessible — CI green (runs #1–#3 `success` on `main`: full suite, style gate, report + `gh-pages` deploy) and the Allure report is **live**: `https://romanmakarenko.github.io/JavaPlaywrightAIFramework/` (Pages enabled 2026-08-23 via API, source `gh-pages`, summary 11/10/1 verified over HTTPS). PR `smoke` path runs the identical command (`-Dgroups=smoke`); optionally verify with a test PR.
...
- **CI green + report live, DoD 6 closed 2026-08-23** (repo pushed to `git@github.com:RomanMakarenko/JavaPlaywrightAIFramework.git`): GitHub runs **#1–#5 all `success`** on `main` (push + workflow_dispatch). Job steps pass: style-check gate → `mvn clean test` (full suite) → `mvn allure:report` → artifacts (`allure-report`, `allure-results`) → `pages` job deployed the report to `gh-pages`. **Pages enabled via API** (`POST /pages`, source `gh-pages` `/`) and the report is **live at `https://romanmakarenko.github.io/JavaPlaywrightAIFramework/`** (verified: title "Allure", HTTP 200). After run #5, `summary.json` = **11 total / 11 passed / 0 skipped / status passed** — the valid-user login test runs and passes (`TEST_USER_EMAIL`/`TEST_USER_PASSWORD` repo secrets set 2026-08-23 and mapped into the CI test-step `env:` via ci.yml, which GitHub Actions requires — secrets are not injected automatically). Optional remaining: verify the PR `smoke` path with a test PR.
...
screenshot** via the REST API. Works locally and in GitHub Actions. Security-first: the API token is a
secret — never committed, logged, or rendered into the (published) Allure report. Full decomposition
+ acceptance criteria: **this section** (the original `TASK_SPECK.md` was never committed — it carried
the exposed API token — and has been removed; its content lives here).
...
- [x] **15.6 Verify** — **done 2026-08-23** on the real site: temporary failing probe (wrong-password login) → `JiraBugListener` filed **`SCRUM-10`** (`[AUTO-TEST] com.funtime.tests.JiraProbeTest#login_wrongPassword_redirectsToHome`, type **Task**, label `auto-test`, **`failure-screenshot.png` attached**); a second run **reused** `SCRUM-10` (comment "Failed again" + fresh screenshot, no duplicate — `SCRUM-12` absent); wrong/truncated token → suite stays green (warning only). Probe test removed, full suite 11/11 green, style-check passes. **Gotchas recorded:** the SCRUM (team-managed) scheme has **no `Bug` type** — only Epic/Subtask/Task/Story — so the first ticket used `jira.issue.type=Task`; the token's trailing `=90AE6DB0` (base64) is part of the token — reading it with `cut -d= -f2` truncates it and yields confusing 401/permission errors (Java `Properties` reads it correctly). **Owner to-dos now done:** CI secrets set (`JIRA_BASE_URL`/`JIRA_EMAIL`/`JIRA_API_TOKEN`/`JIRA_PROJECT_KEY`/`JIRA_ISSUE_TYPE`), Phase 15 committed (`9507502`) and pushed — CI run #6 green on main with tickets enabled, no spurious tickets filed. **Updated 2026-08-23 — switched to literal `Bug`:** the REST API cannot add an issue type to a team-managed scheme (probed both `POST /rest/api/3/issuetype` global create and `/project/SCRUM/issuetype` — the global type did **not** appear in SCRUM's createmeta, then was deleted again), tracked as JSWCLOUD-23545. So **`Bug` must be added via the UI**: Project settings → Issue types → Add issue type → "Bug". Both the local `jira.issue.type` and the CI `JIRA_ISSUE_TYPE` secret are now set to `Bug`. **`Bug` added 2026-08-23 and verified via API** (`jira_get_project_issue_types` → SCRUM has Bug id `10039`) — auto-filed tickets are now literal `Bug` issues (the settings URL for team-managed projects is `/jira/software/projects/<KEY>/settings/issuetypes`, not `/jira/projects/<KEY>/...`). **Still required from the owner:** **rotate the token** exposed in the original `TASK_SPECK.md` (and in untracked `EVIDENCE.md` — never commit those two files).
- [x] **15.7 CI probe** — **done 2026-08-23**, verified end-to-end locally: dedicated probe that verifies the pipeline on CI without touching normal runs. `src/test/java/com/funtime/tests/JiraCiProbeTest.java` (group `ci-jira-probe`) **skips** unless `JIRA_CI_PROBE=true` **and** `ConfigReader.isJiraConfigured()`, then deliberately fails; `src/test/resources/testng-jira-probe.xml` (same listener set incl. `JiraBugListener`); Maven profile `-Pjira-probe` swaps surefire to that suite; `ci.yml` gets a `run-jira-ci-probe` dispatch input that sets `JIRA_CI_PROBE=true`, forces `JIRA_TICKETS_ENABLED`, and runs `-Pjira-probe`. **Verified locally:** unarmed → `Tests run: 1, Skipped: 1` green; armed → probe failed on the deliberate assertion (site loaded, `expected: "CI-JIRA-PROBE…"` vs real title) → `JiraBugListener` filed **`SCRUM-16`** (literal **Bug**, label `auto-test`, `failure-screenshot.png` 414 KB); second armed run **reused** `SCRUM-16` (no duplicate). All probe tickets (`SCRUM-16`, stale `SCRUM-10/14/15`) cleaned from Jira afterwards. Dispatch on CI: `run-jira-ci-probe=true` + `create-jira-tickets=true` — the run intentionally goes red and files the Bug ticket.
