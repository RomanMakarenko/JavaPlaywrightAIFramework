...
Current position: **Phase 12 implemented** — Definition of Done verified (see the done-note below). One acceptance item deferred to the first real GitHub run (the folder is not a git repository yet). Next: Phase 13 (future funtime.com.ua work, outside skeleton scope).
...
- **⚠️ Prerequisite — the folder is not a git repository**, so GitHub Actions cannot run until it is pushed to GitHub:
  `git init && git add -A && git commit -m "…" && git remote add origin <url> && git push -u origin main`, then enable Pages in repo settings (Settings → Pages → `Deploy from a branch` → `gh-pages`). The first real PR/main run then closes the acceptance criteria above (also tracked in Phase 12 DoD).
...
- **Remaining before "CI green":** `git init && git add -A && git commit -m "…"`, create the GitHub repo, `git remote add origin <url> && git push -u origin main`, enable Pages (`gh-pages` branch), optionally set `TEST_USER_EMAIL`/`TEST_USER_PASSWORD` secrets. Then re-check this DoD item.
